...
void TIM2_Configuration(void)
{
  ...
  RCC->APB1ENR |= RCC_APB2ENR_TIM2EN;  // enable Timer2
 
  TIM2->PSC = PRESCALE - 1;
  TIM2->DIER |= TIM_DIER_CC1IE | TIM_DIER_CC2IE;;// | TIM_DIER_CC2IE;		// enable 2 CC interrupt channels
  TIM2->CCMR1 = 0;                     // chan 1 is output
  TIM2->CR1 = TIM_CR1_CEN;             // enable timer
 
  NVIC->ISER[0] = (1 << TIM2_CC_IRQn); // enable TIM2 int in NVIC
